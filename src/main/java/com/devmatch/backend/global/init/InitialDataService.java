...
public class InitialDataService {
  ...
  public void createInitialApplications() {
    if (applicationRepository.count() > 0) {
      return;
    }

    User applicant = userRepository.findAll().get(1); // User 2
    Project project = projectRepository.findAll().getFirst(); // Project 1

    List<Skill> skills = List.of(
        new Skill("Java", 8),
        new Skill("Spring Boot", 7)
    );

    Analysis analysis = analysisService.createAnalysis(project, skills);

    applicationService.createApplication(applicant, project, skills, analysis);
  }
}
